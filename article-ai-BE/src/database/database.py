...
class Database:
    ...
    def _map_config(self, config: Dict | None) -> Dict:
    ...
    def _map_execution_log(self, execution_log: Dict | None) -> Dict:
        """Map the execution log to the database schema"""
        if execution_log == None:
            return {}
        return {
            'id': execution_log[0],
    ...
    def get_blog_post(self, blog_post_id: int) -> Dict:
        ...
        conn.close()
        return self._map_blog_post(blog_post)
    
    def update_blog_post(self, blog_post_id: int, content: str):
        """Update an existing blog post in the database"""
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        cursor.execute('UPDATE blog_posts SET content = ?, updated_at = ? WHERE id = ?', (content, datetime.now(), blog_post_id))
        conn.commit()
        conn.close()

    def _map_blog_post(self, blog_post: tuple) -> Dict:
    ...
    def get_blog_post_by_execution_id(self, execution_log_id: int) -> Optional[Dict]:
        """Get a blog post by execution log ID"""
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        cursor.execute('''
            SELECT id, content, config_id, execution_log_id, created_at, updated_at 
            FROM blog_posts 
            WHERE execution_log_id = ?
        ''', (execution_log_id,))
        blog_post = cursor.fetchone()
        conn.close()
        return self._map_blog_post(blog_post) if blog_post else None
